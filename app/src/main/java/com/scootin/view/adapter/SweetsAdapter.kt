...
class SweetsAdapter (
...
) : PagingDataAdapter<ProductSearchVO, DataBoundViewHolder<AdapterItemAddSweetsBinding>>(diffCallback = object : DiffUtil.ItemCallback<ProductSearchVO>() {
    ...
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): DataBoundViewHolder<AdapterItemAddSweetsBinding> {
        val binding = AdapterItemAddSweetsBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        binding.price.paintFlags= Paint.STRIKE_THRU_TEXT_FLAG
        return DataBoundViewHolder(binding)
    }

    override fun onBindViewHolder(
        holder: DataBoundViewHolder<AdapterItemAddSweetsBinding>,
        position: Int
    ) {
